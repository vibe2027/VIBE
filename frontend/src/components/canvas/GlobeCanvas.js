...
export const GlobeCanvas = () => {
  ...
  useEffect(() => {
    ...
    function rotPt(p) {
      ...
      const sy = Math.sin(rotYRef.current);
      const cx2 = Math.cos(rotXRef.current);
      const sx2 = Math.sin(rotXRef.current);
    ...
    function drawGlobe() {
      ...
      for (let lat = -60; lat <= 60; lat += 30) {
        for (let lon = -180; lon < 180; lon += 5) {
          const p1 = ll2xyz(lat, lon, R);
          const p2 = ll2xyz(lat, lon + 5, R);
          const r1 = rotPt(p1);
          const r2 = rotPt(p2);
          if (r1.z > 0 && r2.z > 0) {
            ctx.beginPath();
      ...
      for (let i = 0; i < 240; i++) {
        const lon = -180 + i * 1.5;
        const p1 = ll2xyz(0, lon, R);
        const p2 = ll2xyz(0, lon + 1.5, R);
        const r1 = rotPt(p1);
        const r2 = rotPt(p2);
        if (r1.z > 0 && r2.z > 0) {
          ctx.beginPath();
          ...
          ctx.lineTo(cx + r2.x, cy + r2.y);
          ctx.shadowBlur = 6;
          ctx.shadowColor = rainbow[Math.floor(i / 30) % rainbow.length];
          ctx.strokeStyle = rainbow[Math.floor(i / 30) % rainbow.length];
          ctx.lineWidth = 2.8;
          ctx.stroke();
          ctx.shadowBlur = 0;
        }
      ...
      cities.forEach((city, ci) => {
        const p = ll2xyz(city.lat, city.lon, R);
        const r = rotPt(p);
        if (r.z > 0) {
          const sx = cx + r.x;
          const sy = cy + r.y;
          const pulse = 0.65 + 0.35 * Math.sin(frameRef.current * 0.07 + ci * 1.3);
          ctx.shadowBlur = 14;
  ...
  const handleMouseMove = (e) => {
    ...
    velXRef.current = (e.clientX - lastXRef.current) * 0.005;
    velYRef.current = (e.clientY - lastYRef.current) * 0.005;
    rotYRef.current += velXRef.current;
    rotXRef.current += velYRef.current;
    rotXRef.current = Math.max(-1.3, Math.min(1.3, rotXRef.current));
    lastXRef.current = e.clientX;
  ...
  const handleTouchMove = (e) => {
    if (!isDragging) return;
    e.preventDefault();
    velXRef.current = (e.touches[0].clientX - lastXRef.current) * 0.005;
    velYRef.current = (e.touches[0].clientY - lastYRef.current) * 0.005;
    rotYRef.current += velXRef.current;
    rotXRef.current += velYRef.current;
    rotXRef.current = Math.max(-1.3, Math.min(1.3, rotXRef.current));
    lastXRef.current = e.touches[0].clientX;
